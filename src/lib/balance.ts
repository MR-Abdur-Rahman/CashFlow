...
export function bilateralBalance(
  splits: any[], settlements: any[], target: any, currentUserId: string | null, myPersonIds: string[], cutoff?: string,
): number {
...
export function settlementNetAfter(
  splits: any[], settlements: any[], settlement: any, currentUserId: string | null, myPersonIds: string[],
): number | null {
  ...
  if (settlement.created_by === currentUserId) {
    target = { id: settlement.person_id, linked_user_id: settlement.person?.linked_user_id ?? null };
  } else {
    target = { id: undefined, linked_user_id: settlement.created_by };
  }
  if (!target.id && !target.linked_user_id) return null;
  return bilateralBalance(splits, settlements, target, currentUserId, myPersonIds, settlement.created_at);
}

      ...
      <Section
        ...
        action={
          <div className="flex gap-1">
            <Button size="sm" variant="ghost" onClick={() => setScanOpen(true)} aria-label="Scan QR">
        ...
        {people.length === 0 ? <Empty text="No people yet" /> : (
          <ul className="divide-y divide-border">
            {people.map((p) => {
              ...
              return (
                <li key={p.id}>
                  <Link to={`/split/person/${p.id}`} className="flex items-center gap-3 p-4 active:bg-secondary/40">
                    <Avatar name={p.name} />
      ...
      >
        {groups.length === 0 ? <Empty text="No groups yet" /> : (
          <ul className="divide-y divide-border">
            {groups.map((g: any) => (
              <li key={g.id}>
                <Link to={`/split/group/${g.id}`} className="flex items-center gap-3 p-4 active:bg-secondary/40">

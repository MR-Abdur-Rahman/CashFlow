...
export default function HistoryPage() {
  ...
  const { data: settlements = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("settlements")
        .select(
          "*, person:person_id(name, nickname, avatar_url, linked:linked_user_id(id, full_name, avatar_url)), creator:created_by(full_name, avatar_url), accounts:account_id(label, institution), split_shares:split_share_id(person_name, share_amount, person:people(linked_user_id)), splits:split_id(description, paid_by, created_by, creator:created_by(full_name), paid_by_person:paid_by_person_id(linked_user_id, name))",
        )
  ...
  const filteredSettlements = useMemo(() => {
    ...
    return (settlements as any[]).filter((s) => {
      ...
      const hay = [
        s.description,
        s.splits?.description,
        s.person?.name,
        s.split_shares?.person_name,
        s.creator?.full_name,
        s.method,
        s.accounts?.label,
        s.accounts?.institution,
  ...
  return (
    <div className="px-4 pt-4 pb-24 space-y-4">
      ...
      <Input
        placeholder="Search by person, phone, account, category, note..."
        value={q}
      ...
      {grouped.map(([date, items]) => (
        <div key={date}>
          <p className="text-xs uppercase text-muted-foreground mb-2 px-1 font-mono">
            {format(new Date(`${date}T00:00`), "MMM dd, yyyy")}
          </p>
            ...
            {items.map((item) =>
              item._kind === "split" ? (
                ...
                <SwipeRow
                  ...
                  canEdit={item.created_by === item._uid}
                  canDelete={canDeleteSettlement(item, item._uid)}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this transaction and update your account balance. This
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function HistorySettlementRow({ s, all }: { s: any; all: any[] }) {
  ...
  const netAfter =
    settlementNetAfter(
      ...
      s,
      balanceData?.currentUserId ?? null,
      balanceData?.myPersonIds ?? [],
    ) ?? undefined;
  return (
    <SettlementRow
  ...
  if (isSplit && t.split) {
      ...
      s.type === "group" && s.groups?.name
        ...
        : s.type === "individual" && s.people?.name
          ...
          : (s.split_shares ?? [])
            ...
            s.description ||
            "Split";
    const totalShares = (s.split_shares ?? []).reduce(
      (sum: number, sh: any) => sum + Number(sh.share_amount),
      0,
    );
    ...
    return (
      <div className="flex items-center gap-3 p-3 bg-card">
        <div
          className={`h-9 w-9 rounded-full bg-secondary flex items-center justify-center ${color}`}
        >
        ...
        <div className="text-right">
          ...
          <p className="text-[10px] text-muted-foreground font-mono">
            {formatMoney(Number(s.total_amount))}
          </p>
        </div>
      </div>
    );
